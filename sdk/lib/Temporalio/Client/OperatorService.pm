# ABOUTME: Raw OperatorService handle (spec R91): one snake_case method per
# ABOUTME: rpc (search-attribute management, nexus endpoints, clusters),
# ABOUTME: generated from the proto service descriptor, each a single
# ABOUTME: unretried bridge rpc-call. Python parity _client.py:316-318.
use v5.38;
use warnings;
use feature 'class';
...
class Temporalio::Client::OperatorService {
    ...
    method call {
        ...
        return $connection->rpc_call(
            $rpc, $request, service => 'operator', %opts);
    }
}
...
Temporalio::Client::OperatorService - raw operator service RPC handle

=head1 SYNOPSIS

    my $op = $client->operator_service;

    await $op->add_search_attributes(
        Temporalio::Core::Proto::resolve(
            'temporal.api.operatorservice.v1.AddSearchAttributesRequest')
            ->new({
                namespace         => 'default',
                search_attributes => { MyAttr => 2 },    # 2 = Keyword
            }));

=head1 DESCRIPTION

The low-level escape hatch over the OperatorService RPC surface (spec R91;
Python parity C<_client.py:316-318>): one snake_case method per rpc in
C<temporal.api.operatorservice.v1.OperatorService> (search-attribute
management via C<add_search_attributes>, C<remove_search_attributes>, and
C<list_search_attributes>, namespace deletion, nexus endpoint CRUD, and
cluster management), generated from the vendored proto service descriptor by
...
C<temporal/api/operatorservice/v1/service.proto>.

Obtained from C<< $client->operator_service >> (or
C<< $connection->operator_service >>); not user-built.
