...
- [x] B8.2: revert sample workarounds per the cleanup contract. All 11 workarounds are now reverted and verified live un-gated in samples-perl (passes @3b3dded, @0195c34, @8f8dbf9): #1 sync-activity, #2 mutex, #3 batch-sliding-window, #4 updatable-timer, #5 external-workflow, #6 cancellation, #7 timer, #8 waiting-for-handlers, #9 local-activity, #10 context-propagation, #11 nexus. The activity-choice sample is fixed (B8.1). The final pass reverted #10 (context-propagation, un-gated by SDK fix B12) and #11 (nexus, un-gated by SDK fix B13); both live smokes pass with the SDK on PERL5LIB.
- [x] B8.3 Update sdk-perl CLAUDE.md + README status lines to "verified live against a dev server" (done: CLAUDE.md "What this is" and README status block now state v0.2.0 is feature-complete and verified live against a dev server, with unit, replay, and live integration suites green). This is genuinely true now: the SDK's own t/integration repros (B3/B4/B5/B6/B7/B9-real-diagnosis/B10/B11/B12/B13) pass live, and all 11 samples-perl live smokes pass un-gated with this SDK.
- [x] B8.4 (Optional) Doc note added to Temporalio::Workflow POD ("RAISING ERRORS FROM WORKFLOW CODE"): business/validation failures should raise Temporalio::Exception::Application with non_retryable => 1 where appropriate, because a plain die in workflow code becomes a RETRYABLE workflow-task failure (the activity-choice lesson). The optional runtime diagnostic was skipped (lower priority, not trivially testable); doc note only.
- [x] B8.5 Verify: sdk-perl full `prove -lj4 t` green; samples-perl `just check` green offline AND every reverted live smoke passes with the SDK on PERL5LIB (from the samples-perl reconciliation @8f8dbf9)
...
- [x] B12.1 RED: unit/integration repro asserting (a) the activity-inbound ExecuteActivity input carries the Start task's header_fields (proto field 6), AND (b) a header set at start round-trips to the activity-inbound hook un-double-encoded; fails first for the documented reasons (header ABSENT on activity-inbound; where present, its value is a doubly-encoded Payload)
- [x] B12.2 GREEN (gap A): thread the activity Start `header_fields` into the ExecuteActivity interceptor input in ActivityDispatcher.pm `_handle_start` (sync branch ~L158, async branch ~L182) so `$input->headers` is populated in the activity-inbound hook
- [x] B12.3 GREEN (gap B): settle the interceptor-header representation so headers pass through as Payloads consistently in BOTH directions (outbound and inbound), matching sdk-python; do not run an already-Payload header value back through `to_payload` (no double-encode)
- [x] B12.4 REFACTOR: one header-representation contract shared by the outbound and inbound paths (pass-through Payload, never re-encoded); comment cites #10
- [x] B12.5 Verify: the activity-inbound header repro passes; B7's workflow_inbound_interceptor.t and B11's workflow_inbound_headers.t still pass; full `prove -lj4 t` green

## B13 C-NEXUS-CALLBACK: :WorkflowRunOperation async completion callback (#11)
- [x] B13.1 RED: subprocess-guarded live repro of a `:WorkflowRunOperation` caller->handler round-trip asserting the caller's operation resolves with the backing workflow's result; today it parks forever / times out (server shows `Pending Nexus Operations: 1`; backing workflow's `WorkflowExecutionStarted` has `completionCallbacks: null`). DONE: `t/integration/repro_nexus_callback.t` (+ fixtures `WfDef::NexusWorkflowRunCaller`, `NexusDef::WorkflowRunHandler`, `WfDef::NexusBackingWorkflow`). Pre-fix RED: the caller never resolves (child died non-zero on the parked-operation teardown).
- [x] B13.2 Investigate: whether the pinned c-bridge / proto already lets Perl populate the StartWorkflowExecution request's `completion_callbacks` (+ Nexus operation token / workflow event links) from the Nexus operation context (likely yes, Perl-side only, like B6's Nexus poll which needed no shim); if shim work IS required, follow the CLAUDE.md memory guard. DONE: PURELY PERL, NO SHIM. Inbound `StartOperationRequest` already carries `callback`/`callback_header`/`request_id`/`links`; `StartWorkflowExecutionRequest` already exposes `completion_callbacks`(21)/`links`(24)/`on_conflict_options`(26) and `Callback.Nexus{url,header}`. No cargo/cbindgen/Alien.
- [x] B13.3 GREEN: attach the Nexus async completion callback (+ operation token + workflow event links) to the WorkflowRunOperation backing-workflow start in OperationContext.pm `WorkflowRunOperationContext::start_workflow` so the server notifies the caller on completion. DONE: dispatcher copies inbound callback/header/request_id/links onto the context; `start_workflow` builds the `common.v1.Callback{nexus,links}`, converts caller links via new `Temporalio::Nexus::Link`, and passes `completion_callbacks`/`links`/`request_id` to the client, which sets `on_conflict_options` when a callback is present. Operation token already worked.
- [x] B13.4 REFACTOR: the callback/token/links wiring lives in one place on the WorkflowRunOperation start path; comment cites #11. DONE: all wiring is in `WorkflowRunOperationContext::start_workflow` (one place) with a #11 comment; link conversion isolated in `Temporalio::Nexus::Link` (MUST-match sdk-python `nexus/_link_conversion.py`).
- [x] B13.5 Verify: the live round-trip repro passes (caller's operation resolves with the backing workflow's result); B6's repro_nexus.t still passes; full `prove -lj4 t` green (all three nexus samples use WorkflowRunOperation, so this unblocks all three). DONE: GREEN repro passes (resolves with 'Hello, world!'); new server-free unit `t/unit/nexus_callback.t` passes; full suite green; ROOT-CAUSE-MAP.md marks #11 resolved (fd theory dead).
