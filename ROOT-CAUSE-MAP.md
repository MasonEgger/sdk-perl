...
| #11 nexus `:WorkflowRunOperation` async parks forever for the caller | B13 C-NEXUS-CALLBACK | RESOLVED 2026-06-26 (B13). DIAGNOSIS: PURELY PERL, NO SHIM (the B6-poll precedent held). The inbound Nexus `StartOperationRequest` already carries everything needed — `callback` (URL), `callback_header`, `request_id`, and `links` (proto fields 3-7 of `temporal.api.nexus.v1.StartOperationRequest`) — and the pinned c-bridge / proto already exposes `completion_callbacks` (field 21), `links` (24), and `on_conflict_options` (26) on `StartWorkflowExecutionRequest`, plus `temporal.api.common.v1.Callback.Nexus{url,header}`. The gap was entirely that the dispatcher never copied those inbound fields onto the context and `WorkflowRunOperationContext::start_workflow` issued a plain `$client->start_workflow` (so `WorkflowExecutionStarted.completionCallbacks` was null and the caller's operation parked: `Pending Nexus Operations: 1`). The operation token already worked (the dispatcher tokenizes the returned `Nexus::WorkflowHandle`). FIX (one commit, no cargo/cbindgen/Alien): (1) `NexusDispatcher::_handle_start` copies `$start->{callback,callback_header,request_id,links}` onto the `WorkflowRunOperationContext` (links normalized to `{url,type}`); (2) `OperationContext.pm` `start_workflow` builds a `common.v1.Callback{nexus=>{url,header}, links}`, converts the inbound caller links via the new `Temporalio::Nexus::Link` (MUST-match sdk-python `nexus/_link_conversion.py`), and passes `completion_callbacks` + `links` + the Nexus `request_id` to the client; (3) `Client::_populate_start_request` places `completion_callbacks`/`links`/`request_id` on the request and sets `on_conflict_options{attach_request_id,attach_completion_callbacks,attach_links}` when a callback is present (MUST-match sdk-python `_build_start_workflow_execution_request`). Live round-trip (`t/integration/repro_nexus_callback.t`, subprocess-guarded) resolves with the backing workflow's result. The fd theory for #11 is CONFIRMED DEAD: #11 shared no fd path with #1 and there was no SDK fd bug — the real cause was the missing async completion callback, exactly as B8 pass-2 instrumented. | FIXED: `sdk/lib/Temporalio/Nexus/OperationContext.pm` (`WorkflowRunOperationContext`), `sdk/lib/Temporalio/Worker/NexusDispatcher.pm` (`_handle_start`), `sdk/lib/Temporalio/Client.pm` (`_populate_start_request`), new `sdk/lib/Temporalio/Nexus/Link.pm`. | Perl (NO shim) | subprocess-guarded integration (live round-trip) + server-free unit (`t/unit/nexus_callback.t`) |
