...
- [x] P10.0.5 Second, distinct flake: signals_queries.t intermittently fails a subtest with a transient gRPC-transport error when -j4 unit-suite load starves the query-answering worker mid-call (load artifact; client RPC timeout defaults to 0 = no client deadline). Two shapes seen: `RpcTimeout`/`DEADLINE_EXCEEDED` and `h2 protocol error: http2 error` (RpcError catch-all). RED: t/unit/test_worker_retry.t. GREEN: add `Temporalio::Test::Worker::await_idempotent($producer, attempts=>3, backoff=>0.5)` — bounded retry that re-issues the RPC on a transient load-stall transport failure only (DEADLINE_EXCEEDED, UNAVAILABLE, or an h2/http2/connection-reset/broken-pipe/transport-error RpcError; classifier `_is_transient_load_error`); any real non-transient error (NOT_FOUND, QueryRejected, …) surfaces immediately. For IDEMPOTENT reads only. Audit all t/integration/*.t: wrap idempotent reads (query, fetch result, schedule describe poll) in await_idempotent; leave start_workflow/signal/terminate (non-idempotent) un-retried. Bump test-harness ceilings (not production timeouts): await_future 30→60, await_with_worker / Test::Worker await_result 60→120, shutdown 60→120, schedule await_num_actions 30→60. Verify: full prove -lj4 t 8x consecutive all exit 0 under forced CPU contention
- [x] P10.0.6 Third + fourth mode, one centralized fix to stop the whack-a-mole. (a) New connect mode: an ephemeral dev server passes its 60s start_timeout but binds/accepts a moment after `Temporalio::Client->connect`, so core's ~5s connect window expires against a not-yet-listening socket and connect fails ("Connection failed: ...ConnectionRefused...", "tcp connect error", "Failed connecting to test server", UNAVAILABLE, TonicTransportError). (b) Recurrence of the P10.0.5 timeout mode on the NON-idempotent calls (start_workflow/signal/terminate) that P10.0.5 left un-retried. RED-first: extend t/unit/test_worker_retry.t (classifier now matches the connect-race shapes; new `await_with_retry` + `start_workflow_with_retry`) and add t/unit/test_client_connect_retry.t. GREEN: extend the ONE shared `_is_transient_load_error` classifier to also match the connect shapes (connect refused / tcp connect error / failed connecting to / TonicTransport), reused by connect + read + teardown — no forked logic; add `Temporalio::Test::Client::connect_with_retry($loop, $connector, attempts=>10, backoff=>1.5)` and route every integration client connect through it (genuine TLS-mismatch / bad-PEM failures still surface at once); add `Temporalio::Test::Worker::await_with_retry` (retries a transient timeout on non-idempotent calls, tolerates WorkflowAlreadyStarted on a start retry as proof the prior RPC landed) + `start_workflow_with_retry` convenience, route all integration start_workflow/signal/terminate sites through them; raise the generous await_idempotent default ceiling 3→6 attempts, 0.5→1.0s backoff. Production SDK semantics untouched (Test:: helpers only). Verify: full prove -lj4 t 12x consecutive all exit 0, 4 under forced CPU contention
